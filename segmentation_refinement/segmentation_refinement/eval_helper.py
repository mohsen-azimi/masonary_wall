...
def process_high_res_im(model, im, seg, L=900):
    ...
    for key, value in images.items():
        # plt.imshow(value[0,0].cpu().numpy())
        # plt.title("G:" + key)
        # plt.savefig("outputs/all_steps/G_" + key + ".png")
        # plt.show()

        # save mask to file using opencv
        # cv2.imwrite(f"outputs/all_steps/G_{key}.png", (value[0,0].cpu().numpy()*255).astype('uint8'))
        pass
    ...
    for new_size in [max(h, w)]:
        ...
        for x_idx in range((w)//step_size+1):
            for y_idx in range((h)//step_size+1):
                ...
                # imshow for debug
                for key, value in grid_images.items():
                    # plt.imshow(value[0,0].cpu().numpy())
                    # plt.title("L: grid_images, " + key)
                    # save to file
                    # save mask to file using opencv
                    # cv2.imwrite(f"outputs/all_steps/L_{key}_{x_idx}_{y_idx}.png",
                    #             (value[0, 0].cpu().numpy() * 255).astype('uint8'))
                    # plt.show()
                    pass
    ...
    # # imshw final results:
    # plt.imshow(images['pred_224'][0,0].cpu().numpy())
    # plt.title('L: fianl fullresolution, pred_224')
    # plt.show()

    # save mask to file using opencv
    # cv2.imwrite("outputs/all_steps/L_final_full_resolution.png", (images['pred_224'][0,0].cpu().numpy() * 255).astype('uint8'))
