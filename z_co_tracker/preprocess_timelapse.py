...
# Open the video file
video_file = r'./assets/S0.0.2.wmv'
...
output_file = './assets/output_time_lapse.mp4'
...
# resize ratio = 0.5
mask = cv2.imread('./assets/S0.0.2.png', cv2.IMREAD_GRAYSCALE)
binary_mask = np.where(mask > 0, 1, 0).astype(np.uint8)*255
cv2.resize(binary_mask, (int(w_new/2), int(h_new/2)))
cv2.imwrite('./assets/output_time_lapse_mask.png', binary_mask)